...
class RestController(object):
    ...
    def updateStateCommand(self, msg, state, command):
        # local body position
        state.body_local_position[0] = msg.axes[7] * 0.03 # lean forward
        state.body_local_position[1] = msg.axes[6] * 0.03 # lean sideways
        state.body_local_position[2] = msg.axes[1] * 0.03 # height

        # local body orientation
        state.body_local_orientation[0] = msg.axes[0] * 0.4 # pivot
        state.body_local_orientation[1] = msg.axes[2] * 0.5 # 4
        state.body_local_orientation[2] = msg.axes[3] * 0.4 # twist

        rospy.loginfo(f"Rest Joystick Axes: {msg.axes}")


        # state.body_local_position[0] = msg.axes[4] * 0.04
        # state.body_local_position[1] = msg.axes[3] * 0.03
        # state.body_local_position[2] = msg.axes[0] * 0.03

        # state.body_local_orientation[0] = msg.axes[0] * 0.4
        # state.body_local_orientation[1] = msg.axes[7] * 0.5
        # state.body_local_orientation[2] = msg.axes[2] * 0.4
